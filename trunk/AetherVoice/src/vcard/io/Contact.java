...
public class Contact {
	...
	static class RowData {
		RowData(final int type, final String data, final boolean preferred,
	...
	static class OrgData {
		OrgData(final int type, final String title, final String company,
				final String customLabel) {
			this.type = type;
			this.title = title;
	...
	/**
	 * Parse a display name into first, middle and last names. Note: prefixes
	 * and suffixes are not recognized.
	 * 
	 * @param displayName
	 */
	protected void parseDisplayName(final String displayName) {
		if (displayName != null) {
			final Matcher m = Contact.namePattern.matcher(displayName);
			if (m.matches()) {
				if (m.group(1) != null) {
					lastName = m.group(2);
					firstName = m.group(3);
