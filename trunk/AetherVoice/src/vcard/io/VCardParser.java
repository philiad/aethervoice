...
public class VCardParser {
	...
	final static Pattern[] phonePatterns = {
			...
			Pattern
					.compile("[+](972)(2|3|4|8|9|50|52|54|57|59|77)(\\d\\d\\d)(\\d\\d\\d\\d.*)"), };
	...
	final static Pattern quotedPrintableSoftbreakPattern = Pattern
			.compile("(.*)=\\s*$");
	...
	interface handleProp {
		void parseProp(final Contact contact, final String propName,
	...
	// Initializer block
	{
		...
		final handleProp simpleValue = new handleProp() {
			public void parseProp(final Contact contact, final String propName,
					final Vector<String> propVec, final String val) {
				if (propName.equals("FN"))
					...
					final String[] names = StringUtil.split(val, ";");
					switch (names.length) {
					default:
					case 5:
		...
		final handleProp orgHandler = new handleProp() {

			public void parseProp(final Contact contact, final String propName,
					final Vector<String> propVec, final String val) {
				String label = null;
				for (final String prop : propVec) {
					final String[] propFields = StringUtil.split(prop, "=");
					if (propFields[0].equalsIgnoreCase(VCardParser.LABEL_PARAM)
							&& propFields.length > 1)
						label = propFields[1];
				}
				if (propName.equals("TITLE")) {
					...
					if (!setTitle)
						contact.orgs.add(new Contact.OrgData(
								label == null ? ContactMethodsColumns.TYPE_WORK
										: ContactMethodsColumns.TYPE_CUSTOM,
								val, null, label));
				} else if (propName.equals("ORG")) {
					final String[] orgFields = StringUtil.split(val, ";");
				...
				for (final String prop : propVec)
					if (prop.equalsIgnoreCase("HOME")
						...
						if (propFields.length > 1
								&& propFields[0]
										.equalsIgnoreCase(VCardParser.LABEL_PARAM)) {
							label = propFields[1];
							subtype = Contacts.ContactMethodsColumns.TYPE_CUSTOM;
						}
					}
				contact.phones.add(new Contact.RowData(subtype,
						toCanonicalPhone(val), preferred, label));
			}
		});
		...
		propHandlers.put("ADR", new handleProp() {
			public void parseProp(final Contact contact, final String propName,
				...
				boolean preferred = false;
				String label = null;
				int subtype = Contacts.ContactMethodsColumns.TYPE_WORK; // vCard
																		// spec
																		// says
																		// default
																		// is
				...
				final String[] addressFields = StringUtil.split(val, ";");
				final StringBuffer addressBuf = new StringBuffer(val.length());
				if (addressFields.length > 2) {
					addressBuf.append(addressFields[2]);
					final int maxLen = Math.min(7, addressFields.length);
					for (int i = 3; i < maxLen; ++i)
						addressBuf.append(", ").append(addressFields[i]);
				}
				final String address = addressBuf.toString();
				contact.addrs.add(new Contact.RowData(subtype, address,
						preferred, label));
			}
		});
		...
		propHandlers.put("EMAIL", new handleProp() {
			public void parseProp(final Contact contact, final String propName,
				...
				contact.emails.add(new Contact.RowData(subtype, val, preferred,
						label));
			}
		});

		propHandlers.put(VCardParser.IMPROP, new handleProp() {
			public void parseProp(final Contact contact, final String propName,
					final Vector<String> propVec, final String val) {
				boolean preferred = false;
				String label = null;
				...
				for (final String prop : propVec)
					if (prop.equalsIgnoreCase("PREF"))
						...
						if (propFields.length > 1)
							if (propFields[0]
									.equalsIgnoreCase(VCardParser.PROTO_PARAM))
								proto = propFields[1];
							else if (propFields[0]
									.equalsIgnoreCase(VCardParser.LABEL_PARAM))
								label = propFields[1];
					}
				final Contact.RowData newRow = new Contact.RowData(subtype,
						val, preferred, label);
	 ...
	 * @return
	 */
	String toCanonicalPhone(final String phone) {
		for (final Pattern phonePattern : VCardParser.phonePatterns) {
			final Matcher m = phonePattern.matcher(phone);
			if (m.matches())
	 ...
	 */
	public long parseVCard(final Contact contact, final BufferedReader vCard)
			throws IOException {
		...
		while (line != null
				&& !VCardParser.beginPattern.matcher(line).matches()) {
			line = vCard.readLine();
			if (line != null)
				parseLen += line.length() + 2;
		}
		...
		while (line != null) {
			...
			vCard.mark(1);
			int ch;
			for (ch = vCard.read(); ch == (int) ' ' || ch == (int) '\t'; ch = vCard
					.read()) {
				vCard.reset();
				final String newLine = vCard.readLine();
				if (newLine != null) {
					parseLen += 2; // Assume CR-LF line ending;
					final int newLen = newLine.length();
					int pos = 0;
					// Search for first non-whitespace char.
					while (pos < newLen
							&& (newLine.charAt(pos) == ' ' || newLine
									.charAt(pos) == '\t'))
						++pos;
					parseLen += pos;
			...
			final Matcher pm = VCardParser.propPattern.matcher(line);

			if (pm.matches()) {
				final String prop = pm.group(1);
				...
				final Matcher ppm = VCardParser.propParamPattern.matcher(prop);
				if (!ppm.find())
					// Doesn't seem to be a valid vCard property
					continue;

				final String propName = ppm.group(1).toUpperCase();
				final Vector<String> propVec = new Vector<String>();
				String charSet = VCardParser.DEFAULT_CHARSET;
				String encoding = "";
				while (ppm.find()) {
					final String param = ppm.group(1);
					final String paramVal = ppm.group(3);
					propVec.add(param
				...
				if (encoding.equalsIgnoreCase("QUOTED-PRINTABLE")) {
					Matcher softBreak = VCardParser.quotedPrintableSoftbreakPattern
							.matcher(val);
					if (softBreak.matches()) {
						// Multi-line quoted-printable
						final StringBuffer tmpVal = new StringBuffer(softBreak
								.group(1));
						do {
							line = vCard.readLine();
							...
							if ((line == null) || (line.length() == 0))
								// skipRead = true;
								break;
							softBreak = VCardParser.quotedPrintableSoftbreakPattern
									.matcher(line);
							if (softBreak.matches())
					...
					try {
						val = QuotedPrintable.decode(val.getBytes("ASCII"),
								charSet);
					} catch (final UnsupportedEncodingException uee) {

					}
				} else if (encoding.equalsIgnoreCase("BASE64")) {
					final StringBuffer tmpVal = new StringBuffer(val);
					do {
						line = vCard.readLine();

						if ((line == null)
								|| (line.length() == 0)
								|| (!VCardParser.base64Pattern.matcher(line)
										.matches()))
							// skipRead = true;
							break;
						tmpVal.append(line);
				...
				final handleProp propHandler = propHandlers.get(propName);
				if (propHandler != null)
	 ...
	 */
	public static void formatEmail(final Appendable cardBuff,
			final Contact.RowData email) throws IOException {
		cardBuff.append("EMAIL;INTERNET");
		if (email.preferred)
			cardBuff.append(";PREF");

		if (email.customLabel != null) {
			cardBuff.append(";" + VCardParser.LABEL_PARAM + "=");
		...
		cardBuff.append(":").append(email.data.trim()).append(VCardParser.NL);
	}
	 ...
	 */
	public static void formatPhone(final Appendable formatted,
			final Contact.RowData phone) throws IOException {
		formatted.append("TEL");
		if (phone.preferred)
			formatted.append(";PREF");

		if (phone.customLabel != null) {
			formatted.append(";" + VCardParser.LABEL_PARAM + "=");
		...
		formatted.append(":").append(phone.data.trim()).append(VCardParser.NL);
	}
	 ...
	 *            The rowdata containing the actual phone data.
	 */
	public static void formatAddr(final Appendable formatted,
			final Contact.RowData addr) throws IOException {
	 ...
	 *            The rowdata containing the actual phone data.
	 */
	public static void formatIM(final Appendable formatted,
			final Contact.RowData im) throws IOException {
		formatted.append(VCardParser.IMPROP);
		if (im.preferred)
			formatted.append(";PREF");
		...
		if (im.auxData != null)
			formatted.append(";").append(VCardParser.PROTO_PARAM).append("=")
					.append(im.auxData);
		if (!StringUtil.isASCII(im.data))
			formatted.append(";CHARSET=UTF-8");
		formatted.append(":").append(im.data.trim()).append(VCardParser.NL);
	}
	 ...
	 */
	public static void formatOrg(final Appendable formatted,
			final Contact.OrgData org) throws IOException {
		if (org.company != null) {
			...
			if (!StringUtil.isASCII(org.company))
				formatted.append(";CHARSET=UTF-8");
			formatted.append(":").append(org.company.trim()).append(
					VCardParser.NL);
			if (org.title == null)
				formatted.append("TITLE:").append(VCardParser.NL);
		}
		if (org.title != null) {
			if (org.company == null)
				formatted.append("ORG:").append(VCardParser.NL);
			...
			if (!StringUtil.isASCII(org.title))
				formatted.append(";CHARSET=UTF-8");
			formatted.append(":").append(org.title.trim()).append(
					VCardParser.NL);
		}
	}
	 ...
	 */
	public void writeVCard(final Contact contact, final Appendable vCardBuff)
			throws IOException {
		// Start vCard

		vCardBuff.append("BEGIN:VCARD").append(VCardParser.NL);
		vCardBuff.append("VERSION:2.1").append(VCardParser.NL);

		VCardParser.appendField(vCardBuff, "X-IRMC-LUID", contact.syncid);

		vCardBuff.append("N");

		if (!StringUtil.isASCII(contact.lastName)
				|| !StringUtil.isASCII(contact.firstName)
				|| !StringUtil.isASCII(contact.midNames)
				|| !StringUtil.isASCII(contact.preName)
				|| !StringUtil.isASCII(contact.sufName))
		...
		vCardBuff
				...
				.append(";")
				.append(
						(contact.firstName != null) ? contact.firstName.trim()
								: "")
				.append(";")
				.append(
						(contact.midNames != null) ? contact.midNames.trim()
								: "")
				.append(";")
				.append((contact.preName != null) ? contact.preName.trim() : "")
				.append(";")
				.append((contact.sufName != null) ? contact.sufName.trim() : "")
				.append(VCardParser.NL);

		for (final Contact.RowData email : contact.emails)
			VCardParser.formatEmail(vCardBuff, email);

		for (final Contact.RowData phone : contact.phones)
			VCardParser.formatPhone(vCardBuff, phone);

		for (final Contact.OrgData org : contact.orgs)
			VCardParser.formatOrg(vCardBuff, org);

		for (final Contact.RowData addr : contact.addrs)
			VCardParser.formatAddr(vCardBuff, addr);

		for (final Contact.RowData im : contact.ims)
			VCardParser.formatIM(vCardBuff, im);

		VCardParser.appendField(vCardBuff, "NOTE", contact.notes);
		VCardParser.appendField(vCardBuff, "BDAY", contact.birthday);

		if (contact.photo != null) {
			VCardParser.appendField(vCardBuff,
					"PHOTO;TYPE=JPEG;ENCODING=BASE64", " ");
			vCardBuff.append(" ");
			Base64Coder.mimeEncode(vCardBuff, contact.photo, 76, VCardParser.NL
					+ " ");
			vCardBuff.append(VCardParser.NL);
			vCardBuff.append(VCardParser.NL);
		}

		// End vCard
		vCardBuff.append("END:VCARD").append(VCardParser.NL);
	}

	/**
	 * Append the field to the StringBuffer out if not null.
	 */
	private static void appendField(final Appendable out, final String name,
			final String val) throws IOException {
		if (val != null && val.length() > 0) {
			out.append(name);
			if (!StringUtil.isASCII(val))
				out.append(";CHARSET=UTF-8");
			out.append(":").append(val).append(VCardParser.NL);
		}
	}
