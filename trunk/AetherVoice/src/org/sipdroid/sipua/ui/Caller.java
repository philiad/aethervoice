...
public class Caller extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        {
        		if (!AetherVoice.release) Log.i("SipUA:","outgoing call");
        		...
        		boolean sip_type = !SettingsWindow.callType.equals(Settings.VAL_PREF_PSTN);
        		...
        		boolean ask = SettingsWindow.callType.equals(Settings.VAL_PREF_ASK);
				...
				if (sip_type && !force) {
	    			...
	    			String sExPat = SettingsWindow.adv_exclude;
    			...
    			else 
    			{
	        		if (number != null && !intent.getBooleanExtra("android.phone.extra.ALREADY_CALLED",false)) {
	        		    	...
	        		    	if (sp.contains("prefix")) {
	        		    		...
	        		    		String prefix = SettingsWindow.callType;
	    				...
	    				String search = SettingsWindow.adv_search;
...
//	        				}        					
						...
						if (SettingsWindow.callType.equals(Settings.VAL_PREF_SIPONLY))
							force = true;
	    				if (!ask && Receiver.engine(context).call(number,force))
	    					setResultData(null);
	    						...
	    						(callthru_prefix = SettingsWindow.pbxes_callthru2).length() > 0) {
