...
import com.neugent.aethervoice.ui.AetherVoice;
import com.neugent.aethervoice.ui.CallHandler;
import com.neugent.aethervoice.ui.SettingsWindow;
	...
	public static void loadReceiverContext(final Context context) {
...
//							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.sipdroid.sipua.ui.Settings.PREF_SIPRINGTONE, Settings.System.DEFAULT_RINGTONE_URI.toString());
							String sUriSipRingtone = SettingsWindow.ringtoneUri.toString().equals("") ? SettingsWindow.ringtoneUri.toString() : Settings.System.DEFAULT_RINGTONE_URI.toString();
					...
					if (wl == null) {
						...
						wl = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK |
								PowerManager.ACQUIRE_CAUSES_WAKEUP, "AetherVoice.onState");
					}
					wl.acquire();
				...
				case UserAgent.UA_STATE_IDLE:
					...
					System.out.println("IDLE CALL");
					// AetherVoice.showIncall(mContext);
					final Message m = new Message();
					m.what = CallHandler.MSG_INCOMING_CALL;
//					m.what = CallHandler.MSG_END_CALL;
					m.obj = Receiver.mContext;
					AetherVoice.mHandler.sendMessage(m);
					
					
					
					ccConn.log(ccCall.base); //AJ: log to call history
					ccConn.date = 0;
			...
			case UserAgent.UA_STATE_INCALL:
				System.out.println("UA_STATE_INCALL");
					...
					msg.what = CallHandler.MSG_INCOMING_CALL;
					msg.obj = Receiver.mContext;
					AetherVoice.mHandler.sendMessage(msg);
					break;
				case UserAgent.UA_STATE_HOLD:
					...
					message.what = CallHandler.MSG_INCOMING_CALL;
					message.obj = Receiver.mContext;
					AetherVoice.mHandler.sendMessage(message);
					break;
				}
				pos(true);
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			...
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					//!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.sipdroid.sipua.ui.Settings.PREF_REGISTRATION, org.sipdroid.sipua.ui.Settings.DEFAULT_REGISTRATION)) //XXX: ano to?
					!SettingsWindow.notif_reg)
				text = null;
				...
				if (type == MISSED_CALL_NOTIFICATION) {
		        	...
		        	default:
		        		if (type >= REGISTER_NOTIFICATION && mSipdroidEngine != null && type != REGISTER_NOTIFICATION+mSipdroidEngine.pref &&
		        			...
		        			notification.contentIntent = PendingIntent.getActivity(mContext, 0,
		        					createIntent(AetherVoice.class), 0);
		...
		static void updateAutoAnswer() {
        	...
        	if(SettingsWindow.autoAnswerOD &&
				AetherVoice.on(mContext)) {
		...
		public static void pos(boolean enable) {
			...
			pos_gps(false);
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && AetherVoice.on(mContext) &&
						...
						SettingsWindow.pbxes_posurl.length() > 0){ 
	        ...
	        (new Thread() {
				public void run() {
					try {
				        ...
				        URL url = new URL(SettingsWindow.pbxes_posurl+		
				        		"?"+opt);
				        BufferedReader in;
		...
		public static void alarm(int renew_time,Class <?>cls) {
       		if (!AetherVoice.release) Log.i("SipUA:","alarm "+renew_time);
		...
		public static void moveTop() {
			...
			if(AetherVoice.threadFlag){ //We will use this flag to know if aethervoice is running XP
		...
		public static void progress() {
			Log.d("AetherVoice", "onReceiver.progress");
			Log.d("AetherVoice", "checking if IDLE... "+(call_state == UserAgent.UA_STATE_IDLE));
			if (call_state == UserAgent.UA_STATE_IDLE) return;
			int mode = RtpStreamReceiver.speakermode;
			
			Log.d("AetherVoice", "checking if speakermode... "+RtpStreamReceiver.speakermode);
			
			if (mode == -1)
				mode = speakermode();
			if (mode == AudioManager.MODE_NORMAL){
				Log.d("AetherVoice", "checking if speakermode... AudioManager.MODE_NORMAL");
				...
				Log.d("AetherVoice", "checking if speakermode... else");
		...
		public static boolean isFast(int i) {
        	...
        	if (wi != null) {
        		if (!AetherVoice.release) Log.i("SipUA:","isFastWifi() "+WifiInfo.getDetailedStateOf(wi.getSupplicantState())
        				+" "+wi.getIpAddress());
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		if (!on_vpn())
                        ...
                        return SettingsWindow.connection == SettingsWindow.CONNECTION_WLAN;
	        		else
	        			...
	        			return SettingsWindow.connection == SettingsWindow.CONNECTION_VPN;
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		...
        		return SettingsWindow.connection == SettingsWindow.CONNECTION_GPRS;
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
        		...
        		return SettingsWindow.connection == SettingsWindow.CONNECTION_EDGE;
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (!AetherVoice.on(context)) return;
        	if (!AetherVoice.release) Log.i("SipUA:",intentAction);
	        	...
	        	if (SettingsWindow.wifi_select) {
		        	...
		        	if (configurations != null) {
		                ...
		                if (bestconfig != null && bestconfig.priority != maxconfig.priority &&
		               		...
		               		if (!AetherVoice.release) Log.i("SipUA:","changing to "+bestconfig.SSID);
