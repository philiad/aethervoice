...
public class SipdroidEngine implements RegisterAgentListener {
	...
	UserAgentProfile getUserAgentProfile(int index) { //XXX: modified by AJ
		...
		user_profile.username = SettingsWindow.username[index];
		...
		user_profile.passwd = SettingsWindow.password[index];
		...
		//}
		if(SettingsWindow.domain[index].length() == 0){
			user_profile.realm = SettingsWindow.server[index];
		}else{
			user_profile.realm = SettingsWindow.domain[index];
		}
		...
		if(SettingsWindow.from_user[index].length() == 0){
			...
			user_profile.from_url = SettingsWindow.from_user[index];
		}
		
		// MMTel configuration (added by mandrajg) //TODO: add to settingswindow
		...
		user_profile.qvalue = SettingsWindow.mmtel_qvalue;
		...
		user_profile.mmtel = SettingsWindow.mmtel;
		...
		user_profile.pub = SettingsWindow.connection == SettingsWindow.CONNECTION_EDGE ||
	...
	public boolean StartEngine() {
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					SipStack.default_transport_protocols[0] = SettingsWindow.getProtocol(SettingsWindow.protocol);
					
					String version = "AetherVoice/" + AetherVoice.getVersion() + "/" + Build.MODEL;
	...
	void setOutboundProxy(SipProvider sip_provider,int i) {
		try {
			if (sip_provider != null) sip_provider.setOutboundProxy(new SocketAddress(
					...
					Integer.valueOf(SettingsWindow.port)));
	...
	public void updateDNS() {
		...
		for (SipProvider sip_provider : sip_providers) {
			...
			try {
				...
				edit.putString("dns"+i, IpAddress.getByName(SettingsWindow.server[i]).toString());
	...
	public boolean call(String target_url,boolean force) {
		...
		if (!found || (ua = uas[p]) == null) {
			...
			if(SettingsWindow.pbxes_callback &&
	...
	public int speaker(int mode) {
		int ret = 0;
		try{
			ret = ua.speakerMediaApplication(mode);
			Receiver.progress();
